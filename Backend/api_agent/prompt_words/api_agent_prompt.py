...
def small_api_analysis_task_prompt(message):
    prompt = f"""
...
def api_analyzer_system_prompt():
    prompt = """
...
def api_case_design_system_prompt():
    prompt = """
        你是一位专业的API测试用例设计专家，负责根据API分析结果对每一个接口设计全面、高质量的测试用例。
        ...
        2. 描述：测试的目标和预期结果
        3. 接口地址：接口url
        4. 基础URL：被测环境的基础URL，取值 $base_url$
        6. 请求方式：HTTP方法
        5. 前置条件：测试执行的必要准备工作
        6. 测试步骤：详细的操作步骤
        7. 输入数据：请求参数、请求体和必要的请求头
        8. 预期结果：期望的响应状态码、响应体和其他响应属性 (注意，对于非正向场景的测试用例，预期结果只要一个响应状态码即可)
        9. 后置处理：测试完成后的清理步骤
        ...
        **输出要求**:
        请以清晰、有条理的自然语言或Markdown格式输出你设计的测试用例。你的输出将由另一个专门的智能体转换为结构化的JSON，所以你不需要担心严格的JSON语法。

        **请专注于以下内容**:
        - **测试用例的完整性**: 确保覆盖所有核心功能、边界条件和异常场景(确保每个接口至少有2条测试用例)。
        - **逻辑的清晰性**: 每个测试用例的目标、步骤和预期结果都应清晰明了。
        - **内容的准确性**: 确保API端点、HTTP方法和请求/响应的关键信息准确无误。
        """
    return prompt


def api_structure_case_system_prompt(json_format_example):
    prompt = f"""
